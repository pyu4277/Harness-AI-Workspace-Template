...
def create_analysis_template(scene: dict) -> dict:
    ...
    return {
        ...
        "importance_reason": "",
        "importance_rule_version": 0,
    }


def process_single_video(video_stem: str, manifest_base: Path, analysis_base: Path) -> None:
    """단일 영상의 통합 분석 템플릿을 생성한다."""
...
def main():
    ...
    for vdir in video_dirs:
        stem = vdir.name
        print(f"\n--- [{stem}] ---")
        process_single_video(stem, manifest_base, analysis_base)
    ...
    print("  6. workspace/analysis/{stem}/scene_NNN.json 갱신")
    print("=" * 60)
