...
- 파일명: `YYMMDD_[이름]_[태그]` (리서치/로그). **출력 확장자는 입력 양식 확장자 echo** (HWPX→HWPX, DOCX→DOCX, PDF→PDF). 양식 미제공 시 markdown 폴백. 고정 파일명 규칙 폐기 (IMP-038).
- HWPX 양식 채우기는 `scripts/fill_hwpx.py` 사용 (ZIP+section0.xml 패치 + 자수 카운터 자동 갱신, [[HWPX_Form_Fill_Pattern]] concept 참조).
- 회차 작업은 `260414_N차/` 안에서만. 이전 회차(M<N) 수정 금지 (pre_tool_guard.sh WARN).
- 시간 불변 자산(evidence_vault/ 실파일)은 1차에만 canonical 저장. N차(N≥2) 는 evidence_source 포인터로 참조만 (IMP-040).
- 모든 수치·성과·기업 주장은 `evidence_vault/INDEX.md` 또는 `research_cache/`에 인용 링크 필수.
