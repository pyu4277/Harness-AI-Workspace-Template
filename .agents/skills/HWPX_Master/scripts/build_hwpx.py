...
def build(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 1. Copy base template or unpack hwpx
        if hwpx:
            if not hwpx.is_file():
                raise SystemExit(f"HWPX file not found: {hwpx}")
            import zipfile
            with zipfile.ZipFile(hwpx, 'r') as zf:
                zf.extractall(work)
            
            # Apply data substitution if provided
            if data:
                if not data.is_file():
                    raise SystemExit(f"Data file not found: {data}")
                import json
                import xml.sax.saxutils
                sub_data = json.loads(data.read_text(encoding='utf-8'))
                for section_file in work.glob("Contents/section*.xml"):
                    xml_str = section_file.read_text(encoding='utf-8')
                    for k, v in sub_data.items():
                        xml_str = xml_str.replace(k, xml.sax.saxutils.escape(str(v)))
                    section_file.write_text(xml_str, encoding='utf-8')
        else:
...
def main() -> None:
    ...
    parser.add_argument(
        "--hwpx",
        type=Path,
        help="Original HWPX file to auto-fill (Track B)",
    )
    parser.add_argument(
        "--data",
        type=Path,
        help="JSON data file for auto-fill (Track B)",
    )
    parser.add_argument(
        "--output", "-o",
        type=Path,
